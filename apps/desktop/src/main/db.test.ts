...
/** The schema v7 DDL: v6 plus the downloads table. */
const V7_DDL =
  V6_DDL +
...
/** The current (schema v8) DDL: v7 plus the meta.quickBrowseExternal column. */
const V8_DDL =
  V7_DDL +
...
describe("migrate", () => {
  ...
  test("upgrades a v5 database to the current version, adding an empty site_zoom table, the downloads table, and the quick-browse toggle and preserving the search engine and other state", () => {
  ...
  test("upgrades a v6 database to the current version (v8), adding the downloads table and the quick-browse toggle and preserving the site_zoom row and other state", () => {
    const path = join(tempDir, "v6-to-current.db");
    const db = new Database(path);
    db.exec(V6_DDL);
    // Seed enabled=0 and a non-default searchEngine so a spurious re-create/migrate
    // (which would reset them to their defaults) is detectable, plus a site_zoom row
    // that must survive the upgrade untouched.
    db.prepare(
  ...
  test("upgrades a v7 database to the current version (v8), adding the quick-browse toggle and preserving the downloads table and other state", () => {
  ...
  test("creates a fresh v8 schema with enabled=1, the allowlist, history, site_zoom, and downloads tables, the search engine, and the quick-browse toggle on an empty database", () => {
  ...
  test("is a no-op on a database already at the current version (v8), leaving seeded quickBrowseExternal, site_zoom, and downloads rows untouched", () => {
    ...
    // detectable, and site_zoom and downloads rows so a re-create would be
    // observable.
    db.prepare(
      "INSERT INTO meta(id,schemaVersion,activeSpaceId,enabled,searchEngine,quickBrowseExternal) VALUES (0, 8, 'space-9', 0, 'google', 0)",
    ).run();
    db.prepare(
      "INSERT INTO site_zoom(host,factor,updatedAt) VALUES ('example.com', 1.5, 42)",
    ).run();
...
describe("readQuickBrowseExternal / writeQuickBrowseExternal", () => {
  ...
  test("defaults to true when the stored value is NULL", () => {
    ...
    seed.prepare(
      "INSERT INTO meta(id,schemaVersion,activeSpaceId,enabled,quickBrowseExternal) VALUES (0, 8, 'space-x', 1, NULL)",
    ).run();
    seedRows(seed, "space-x");
    seed.close();
    loadStore();

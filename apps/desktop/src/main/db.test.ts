...
/** The current (schema v3) DDL, adding the `blocking_allowlist` table. */
const V3_DDL =
  V2_DDL +
...
describe("migrate", () => {
  test("upgrades a v1 database to the current version, adding enabled=1 and preserving rows", () => {
  ...
  test("upgrades a v2 database to v3, adding the blocking_allowlist table", () => {
    const path = join(tempDir, "v2.db");
    const db = new Database(path);
    db.exec(V2_DDL);
    // Seed enabled=0 so the migration is confirmed to preserve the flag.
    db.prepare(
      "INSERT INTO meta(id,schemaVersion,activeSpaceId,enabled) VALUES (0, 2, 'space-2', 0)",
    ).run();
    ...
    expect(meta.schemaVersion).toBe(3);
    expect(hasAllowlistTable(db)).toBe(true);
    // Pre-existing meta preserved across the upgrade.
    expect(meta.activeSpaceId).toBe("space-2");
    expect(meta.enabled).toBe(0);
    db.close();
  });

  test("creates a fresh v3 schema with enabled=1 and the allowlist table on an empty database", () => {
